...
/****** FILE: themes/maintheme/js/jquery.mousewheel.js *****/
...
/****** FILE: themes/maintheme/js/jquery.jscrollpane.min.js *****/
...
/****** FILE: themes/maintheme/js/scroll1.js *****/
...
var months = ['Januar', 'Februar', 'März', 'April', 'Mai', 'Juni', 'Juli', 'August', 'September', 'Oktober', 'November', 'Dezember'];
  ...
  function AvailabilityCalendar(container, bookedDates) {
    ...
    this.createCalendar(this.date.getFullYear(),this.month);
    //this.renderMonth();
  }
  ...
  AvailabilityCalendar.prototype = {
     ...
     * Setup methods
     */
    __createToolbar: function (op) {
      
      var $toolbar = $('').appendTo(this.container);
      
      
      if(op==1){
      var $inputContainer = $().appendTo($toolbar);
      ...
      $inputs.eq(0).on('click', function () {
        ...
        self.date.setDate(1);
        var ddate = new Date();
        var mmonth=ddate.getMonth();
        $('table').prev().remove();
        $('table').remove();
        self.createCalendar(self.date.getFullYear(),mmonth);
      });

      $inputs.eq(1).on('click', function () {
        var gg=self.date.setFullYear(self.date.getFullYear() - 1);
        var ddate = new Date();
        var yyear=ddate.getFullYear();
        if(yyear==self.date.getFullYear())
        {
        var ddate = new Date();
        var mmonth=ddate.getMonth();
        $('table').prev().remove();
        $('table').remove();
        self.createCalendar(self.date.getFullYear(),11);
        }
        else{
        $('table').prev().remove();
        $('table').remove();
        self.createCalendar(self.date.getFullYear(),00);
        }
      });
    ...
    __createTable: function () {
      var $table = $('<div></div>').appendTo(this.container);
      $table.addClass('monthholder col-6 col-sm-4 col-lg-3');
      var $tableinner = $('<table></table>').appendTo($table);
      $tableinner.addClass('month');
      this.$monthLabel = $('<th class="month-name" colspan="7">Test</th>').appendTo($tableinner).wrap('<tr></tr>');
      // Weekday headers
      var $tr = $('<tr></tr>').appendTo($tableinner);
      ...
      // Day cells
      for (var i = 0; i < 6; ++i) {
        $tr = $('<tr></tr>').appendTo($tableinner);
        $tr.append('<td></td><td></td><td></td><td></td><td></td><td></td><td></td>');
      }

      this.$cells = $tableinner.find('td');
    },
    createCalendar: function (year,mmm) {
      var date,month,imp,op=0;
      date=new Date(year,mmm);
      month=date.getMonth();
      for(imp=month,op=1;imp<=11;imp++,op++){
      this.__createToolbar(op);   //remove?
      this.__createTable();
      this.renderMonth(imp,this.date.getFullYear());

      }
      if(month>0)
      {
      for(var bb=0;bb<month;++bb){
        this.__createToolbar(op);  //remove?
      this.__createTable();
        this.renderMonth(bb,this.date.getFullYear()+1);
      }
      }
    ...
    renderMonth: function (imp,year) {
      var cellIndexes = {};
      var dates = [];
      var date = new Date(year, imp,1);
      this.$monthLabel.html();
      this.$monthLabel.html(months[imp]+" "+year);
      ...
      this.__addThisMonthDays(date, year, imp, cellIndexes, dates);
      this.__addNextMonthDays(date, imp, cellIndexes, dates);
